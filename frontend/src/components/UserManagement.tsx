...
export const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('https://ai-18-highseek-ps02.onrender.com/api/users');
  ...
  const handleDeleteUser = async (username) => {
    ...
    try {
      ...
      const response = await fetch(`https://ai-18-highseek-ps02.onrender.com/api/users/${username}`, {
        method: 'DELETE',
      });
